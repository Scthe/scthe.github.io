...
TODO justify text
TODO css below images
TODO image dialogs
{:/comment}


This is an electronic version of my notes created during [webgl-pbr](//github.com/Scthe/webgl-pbr) project. There may be some mistakes, so be vary. This article focuses on point lights for brevity.
...
* **roughness** -
* **glossiness** -
* **metallic** - the metallic-ness (0 = dielectric, 1 = metallic) of a material. This is a linear blend between two different models. The metallic model has no diffuse component and also has a tinted incident specular, equal to the base color.
...
* **radiance**
* **irradiance** - TODO, Since this article assumes point lights, irradiance is not going to be of much use.
* **reflection** - TODO also talk about connection to diff/spec
* **refraction** - TODO also talk about connection to diff/spec
...
* **h** -  <rewrite> half-vector, between the incoming and outgoing direction: $$h = \frac {l+v}{\|l+v\|}$$
* $$dot(n,l)$$ - dot product of vectors n,l. Usually assumed both n, l are normalized
...
![Microfacets]({{image_dir}}/other/hemi2.jpg)
*Vectors in hemisphere*
...
![Microfacets]({{image_dir}}/diff-spec--scissors.jpg)
*On image a) we see scissors. Image b) shows the extracted diffuse component. It's visible that the handle of the scissors is orange with some grey parts. Specular component is shown on c), and it does not show any color whatsoever. Instead it displays the specular highlights.*


![Microfacets]({{image_dir}}/Lighting of Killzone Shadow Fall--27.jpg)
*Diffuse component does not depend on view angle, while specular does*
...
![Microfacets]({{image_dir}}/other/a1.jpg)
*Notation for BRDF: l is reversed vector of light direction, v is reversed vector of view direction, h is halfway vector between v and l, m is normal of microfacet, n is normal macrosurface*
...
> Microfacets are much smaller then pixel, too small to even represent using normal maps. That's why some of the equations will sometimes resemble statistical distribution.
...
![FresnelAngles]({{image_dir}}/)
**
{::comment} TODO own image: on the left is mirror, that shows only point p reflects into direction v, on the right many microfacets reflects into v {:/comment}
...
{::comment} TODO fix css to provide vertical spacing{:/comment}
...
![Microfacets]({{image_dir}}/other/shadowing.jpg)
**
{::comment} TODO own image {:/comment}

> The value of visibility function is a scalar, constrained between 0-1.

One of the main purposes behind visibility function is energy conservation [9].
There are many models for G(l,v,h). Here are a selected few:
...
* **Smith, per [10]:**
	...
	It's been said that light ray can be blocked between light and fragment, and also between fragment and camera. Smith's equation takes it into consideration and breaks G into two components:
...
![Microfacets]({{image_dir}}/other/Lighting of Killzone Shadow Fall 28.jpg)
**
{::comment} TODO own image {:/comment}
...
![FresnelAngles]({{image_dir}}/other/fresnel_angles.jpg)
**
{::comment} TODO own image {:/comment}
...
> TODO add note on 'remember me': https://www.fxguide.com/featured/game-environments-parta-remember-me-rendering/ or just 'The are and rendering of remember me'
