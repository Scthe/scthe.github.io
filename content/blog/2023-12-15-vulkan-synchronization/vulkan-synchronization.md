...
`VkEvents` are used when all commands after [vkCmdWaitEvents()](https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/vkCmdWaitEvents2.html) have to wait for [vkCmdSetEvent()](https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/vkCmdSetEvent2.html). You can signal/unisgnal events using a command in `command buffer` (`vkCmdSetEvent()`, `vkCmdResetEvent()`) and from the host CPU device (`vkSetEvent()`, `vkResetEvent()`). Only the GPU can wait for an event (`vkCmdWaitEvent`), the host **cannot**. Example:
...
**`VkFence` is a good choice when** you want to block CPU thread till a certain event happens. It's common to use `VkFence` when the CPU would want to begin the work on the next frame. Use `VkFence` to block the CPU and prevent modifying resources still used by previous frames. We will explore the concept of `frames in flight` in the later part of the article.
...
This may seem similar to the `VkFence`. The difference is, `vkDeviceWaitIdle()` and `vkQueueWaitIdle()` do not take an argument that denotes any particular event. Both just wait for all the work to drain.
...
There are several special stages like `TOP_OF_PIPE_BIT` (start of a command) and `BOTTOM_OF_PIPE_BIT` (end of a command). With these 2 values you can express the following dependency: "before we **start** this command (`TOP_OF_PIPE_BIT`), please **finish ANY other thing** you are doing (`BOTTOM_OF_PIPE_BIT`)". That's a suboptimal barrier. Useful for debugging or as a placeholder. There is even an aptly named `ALL_COMMANDS_BIT` if needed.
...
More details are in the specification's [Appendix B](https://registry.khronos.org/vulkan/specs/1.3-extensions/html/chap53.html). It describes the Vulkan memory model. The whole thing is above my pay grade.

To not dwell more on the intricacies of the system, [VkAccessFlagBits2](https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/VkAccessFlagBits2.html) are a part of every barrier. After the write, to make its result `available` and `visible`, we need to provide access scope. This is defined as a tuple of `VkPipelineStageFlagBits2` and `VkAccessFlagBits2`. It can be specified for both 1) an operation that does the write and 2) a subsequent operation. In [VkMemoryBarrier2](https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/VkMemoryBarrier2.html):

* `srcStageMask`, `srcAccessMask`. Define access scope for an operation that does the write. Only `VkAccessFlagBits2` that end with `*_WRITE_BIT` makes sense here. If this operation is a read, you do not need `memory dependency` in the first place. Only `srcStageMask` for `execution dependency` would have sufficed.
...
* it may depend on memory resources used by other `frames in flight`. If you have 2 `frames in flight`, each odd (e.g. N, N+2, N+4) frame will share UBO buffer memory.
...
There are many commands other than `vkCmdDraw()` and `vkCmdDispatch()` that affect GPU memory. And they need a barrier too. Sometimes it's listed in docs e.g. [vkCmdFillBuffer()](https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/vkCmdFillBuffer.html) says:
